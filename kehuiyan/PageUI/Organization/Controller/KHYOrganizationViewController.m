//
//  KHYOrganizationViewController.m
//  kehuiyan
//
...
#import "KHYOrganizationDetailListVC.h"
#import "KHYOrganizationMamberVC.h"

@interface KHYOrganizationViewController ()<UITableViewDelegate,UITableViewDataSource>
@property (nonatomic, strong) NSArray *dataArr;
@property (nonatomic, strong) NSMutableDictionary *sectionHeaderViewStatusDict;
@end
// 设置区头高度
static const CGFloat headerViewHeigh = 50;
@implementation KHYOrganizationViewController
...
- (void)requestData
{
    ...
    [param setValue:@"organization" forKey:@"act"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        // 存在子部门 展开列表展示信息
        KHYOrganizationDetailListVC *detailVC = [[KHYOrganizationDetailListVC alloc] init];
        detailVC.model = model2;
        detailVC.isTask = self.isTask;
        [self.navigationController pushViewController:detailVC animated:YES];
    }
    if (model2.list.count == 0 && model2.list != nil)
    {
        // 不存在子部门 直接跳转新的成员信息列表
        KHYOrganizationMamberVC *memberVC = [[KHYOrganizationMamberVC alloc] init];
        NSInteger dep_id = model2.dep_id;
        memberVC.dep_id = @(dep_id).stringValue;
        memberVC.titleName = model2.name;
        memberVC.isTask = self.isTask;
