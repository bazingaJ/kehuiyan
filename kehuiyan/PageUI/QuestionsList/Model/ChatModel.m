...
- (NSArray<UUMessageFrame *> *)additems:(NSInteger)number
{
    ...
    for (int i=0; i<number; i++) {
        ...
        NSDictionary *dataDic = [self getDicByNumber:i];
        UUMessageFrame *messageFrame = [[UUMessageFrame alloc]init];
        UUMessage *message = [[UUMessage alloc] init];
        [message setWithDict:dataDic];
        [message minuteOffSetStart:previousTime end:dataDic[@"strTime"]];
...
- (NSDictionary *)getDicByNumber:(NSInteger)numbers
{
    ...
    switch (numbers) {
        case 0:
        {
            ...
            [dictionary setObject:@"你好?" forKey:@"strContent"];
        }
            break;
        case 1:
        {
            ...
            [dictionary setObject:@(UUMessageTypeText) forKey:@"type"];
            [dictionary setObject:@"你好！请问有什么可以帮助你的吗？" forKey:@"strContent"];
        }
            break;
        case 2:
        {
            [dictionary setObject:@(UUMessageFromOther) forKey:@"from"];
            [dictionary setObject:@(UUMessageTypeText) forKey:@"type"];
            [dictionary setObject:@"你好！我想咨询一下产后康复。" forKey:@"strContent"];
        }
            break;
        case 3:
        {
            [dictionary setObject:@(UUMessageFromMe) forKey:@"from"];
            [dictionary setObject:@(UUMessageTypePicture) forKey:@"type"];
        }
            break;
        case 4:
        {
            [dictionary setObject:@(UUMessageFromMe) forKey:@"from"];
            [dictionary setObject:@(UUMessageTypeText) forKey:@"type"];
            [dictionary setObject:@"好的。" forKey:@"strContent"];
        }
            break;
