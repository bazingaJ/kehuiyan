...
@interface KHYDatePickerQuarterlyView () {
    NSString *yearStr;
    NSString *quartStr;
}
 ...
 *  月
 */
@property (nonatomic, strong) NSMutableArray *quarterlyArr;

@end

@implementation KHYDatePickerQuarterlyView
 ...
 *  月
 */
- (NSMutableArray *)quarterlyArr {
    if(!_quarterlyArr) {
        _quarterlyArr = [NSMutableArray array];
    }
    return _quarterlyArr;
}
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if(self) {
        ...
        //设置季
        [self.quarterlyArr removeAllObjects];
        [self.quarterlyArr addObject:@[@"1",@"第一季"]];
        [self.quarterlyArr addObject:@[@"2",@"第二季"]];
        [self.quarterlyArr addObject:@[@"3",@"第三季"]];
        [self.quarterlyArr addObject:@[@"4",@"第四季"]];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    switch (tableView.tag) {
        ...
        case 201: {
            //季
            return self.quarterlyArr.count;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    static NSString *cellIndentifier = @"KHYDatePickerQuarterlyViewCell";
    ...
    switch (tableView.tag) {
        ...
        case 201: {
            //季
            NSArray *itemArr = [self.quarterlyArr objectAtIndex:indexPath.row];
            titleStr = [NSString stringWithFormat:@"%@度",itemArr[1]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (tableView.tag) {
        ...
        case 201: {
            ...
            NSArray *itemArr = [self.quarterlyArr objectAtIndex:indexPath.row];
            quartStr = itemArr[0];
            NSString *titleStr = [NSString stringWithFormat:@"%@度",itemArr[1]];
            [btnFunc2 setFrame:CGRectMake(tWidth, 10, tWidth, 30)];
...
- (void)dismiss {
    ...
    //结束时间
    NSString *endTime = @"";
    switch ([quartStr integerValue]) {
        case 1: {
            //第一季度
            startTime = [NSString stringWithFormat:@"%@-1-1",yearStr];
            endTime = [NSString stringWithFormat:@"%@-3-31",yearStr];
            
            break;
        }
        case 2: {
            //第二季度
            startTime = [NSString stringWithFormat:@"%@-4-1",yearStr];
            endTime = [NSString stringWithFormat:@"%@-6-30",yearStr];
            
            break;
        }
        case 3: {
            //第三季度
            startTime = [NSString stringWithFormat:@"%@-7-1",yearStr];
            endTime = [NSString stringWithFormat:@"%@-9-30",yearStr];
            
            break;
        }
        case 4: {
            //第四季度
            startTime = [NSString stringWithFormat:@"%@-10-1",yearStr];
            endTime = [NSString stringWithFormat:@"%@-12-31",yearStr];
