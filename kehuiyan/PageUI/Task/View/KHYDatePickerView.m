...
@implementation KHYDatePickerView
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    static NSString *cellIndentifier = @"KHYDatePickerViewCell";
    ...
    switch (tableView.tag) {
        ...
        case 202: {
            //日
            titleStr = [NSString stringWithFormat:@"%@日",[self.daysArr objectAtIndex:indexPath.row]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (tableView.tag) {
        ...
        case 201: {
            ...
            //获取当前月份天数
            NSInteger dayNum = [self getSumOfDaysInMonth:yearStr month:monthStr];
            [self.daysArr removeAllObjects];
            for (int i=1; i<=dayNum; i++) {
                [self.daysArr addObject:[NSString stringWithFormat:@"%zd",i]];
            }
        ...
        case 202: {
            ...
            //设置按钮3
            dayStr = [self.daysArr objectAtIndex:indexPath.row];
            NSString *titleStr = [NSString stringWithFormat:@"%@日",dayStr];
            [btnFunc3 setFrame:CGRectMake(tWidth*2, 10, tWidth, 30)];
...
- (void)dismiss {
    ...
    if(!IsStringEmpty(yearStr) &&
       !IsStringEmpty(monthStr) &&
       !IsStringEmpty(dayStr)) {
        
        dateStr = [NSString stringWithFormat:@"%@-%@-%@",yearStr,monthStr,dayStr];
    }
